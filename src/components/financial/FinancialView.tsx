...
import { CashEntry, FinancialIntegrationConfig } from '../../types';
...
export const FinancialView: React.FC<FinancialViewProps> = ({
  ...
  const [financialConfig, setFinancialConfig] = useState<FinancialIntegrationConfig | null>(null);
  ...
  // Quick sync config modal (when key is not yet set)
  const [isQuickSyncModalOpen, setIsQuickSyncModalOpen] = useState(false);
  const [quickSbUrl, setQuickSbUrl] = useState('https://dpaylubvupjjokpukuxy.supabase.co');
  const [quickSbKey, setQuickSbKey] = useState('');
  const [quickSbTable, setQuickSbTable] = useState('extra_incomes');
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [entriesRes, summaryRes, configRes] = await Promise.all([
        ...
        api.getFinancialConfig(currentTenantId).catch(() => null),
      ]);

      setCashEntries(entriesRes.entries || []);
      setMonthlySummary(summaryRes);
      if (configRes) {
        setFinancialConfig(configRes);
        if (configRes.supabaseUrl) setQuickSbUrl(configRes.supabaseUrl);
        if (configRes.supabaseKey) setQuickSbKey(configRes.supabaseKey);
        if (configRes.supabaseTable) setQuickSbTable(configRes.supabaseTable);
      }
    } catch (err) {
  ...
  const handleSyncAll = async (forceAll = true) => {
    // If Supabase key is missing, prompt user to confirm/provide key
    if (!financialConfig?.supabaseKey && !quickSbKey) {
      setIsQuickSyncModalOpen(true);
      return;
    }
    ...
    try {
      const res = await api.syncAllPendingCashEntries(currentTenantId, selectedMonth, forceAll);
      setSyncFeedback({
        success: res.success,
        message: res.message,
      });
      ...
      setSyncFeedback({
        success: false,
        message: `Falha na sincronização: ${err?.message || 'Erro inesperado'}`,
      });
    } finally {
  ...
  const handleSyncSingle = async (entryId: string) => {
    if (!financialConfig?.supabaseKey && !quickSbKey) {
      setIsQuickSyncModalOpen(true);
      return;
    }

    setIsSyncingId(entryId);
    setSyncFeedback(null);
    try {
      const res = await api.syncCashEntry(currentTenantId, entryId);
      setSyncFeedback({
        success: res.success,
        message: res.message || (res.success ? 'Lançamento sincronizado com sucesso!' : 'Erro na sincronização'),
      });
      await loadData();
    } catch (err: any) {
      setSyncFeedback({
        success: false,
        message: `Falha ao sincronizar lançamento: ${err?.message || 'Erro inesperado'}`,
      });
    } finally {
      setIsSyncingId(null);
    }
  ...
  const handleSaveQuickConfigAndSync = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!quickSbKey.trim()) {
      alert('Por favor, informe a Chave de Acesso (anon ou service_role key) do Supabase.');
      return;
    }

    setIsSavingQuickConfig(true);
    try {
      await api.saveFinancialConfig(currentTenantId, {
        enabled: true,
        mode: 'SUPABASE',
        supabaseUrl: quickSbUrl.trim(),
        supabaseKey: quickSbKey.trim(),
        supabaseTable: quickSbTable.trim() || 'extra_incomes',
      ...
      setIsQuickSyncModalOpen(false);
      // Run sync immediately for the selected month
      setIsSyncingAll(true);
      const res = await api.syncAllPendingCashEntries(currentTenantId, selectedMonth, true);
      setSyncFeedback({
        success: res.success,
        message: res.message,
      });
      ...
      alert(`Erro ao salvar e sincronizar: ${err?.message || 'Falha inesperada'}`);
    } finally {
  ...
  const totalEffectiveCash = cashEntries.reduce((acc, c) => acc + (Number(c.effectiveAmount) || 0), 0);
  const unsyncedCount = cashEntries.filter(c => !c.syncedToExternal && Number(c.effectiveAmount) > 0).length;
  const syncedCount = cashEntries.filter(c => c.syncedToExternal && Number(c.effectiveAmount) > 0).length;
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'integration' ? (
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            disabled={isSyncingAll || cashEntries.length === 0}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 hover:from-emerald-700 hover:to-teal-800 text-white text-xs font-bold flex items-center gap-2 shadow-xs transition cursor-pointer disabled:opacity-50"
            title="Enviar e somar todos os lançamentos deste mês no app Controle Financeiro"
          >
            {isSyncingAll ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin" />
                <span>Sincronizando Mês...</span>
              </>
            ) : (
              <>
                <Zap className="w-4 h-4 text-amber-300 fill-amber-300" />
                <span>
                  Sincronizar Mês ({new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalEffectiveCash)})
                </span>
              </>
            )}
          </button>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <div className="font-bold text-sm">
                {syncFeedback.success ? 'Sincronização Realizada com Sucesso!' : 'Atenção na Sincronização'}
              </div>
              <div className="mt-0.5 whitespace-pre-line">{syncFeedback.message}</div>
            </div>
          </div>
          <button
            type="button"
      ...
      {/* Integration Status Quick Bar */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
              <span>Integração com Controle Financeiro</span>
              {financialConfig?.supabaseKey || quickSbKey ? (
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
              Destino: <strong>Renda Extra</strong> &middot; Descrição: <strong>MASSOTERAPIA</strong> &middot; Origem: <strong>SERVIÇO</strong> &middot; Tabela: <code className="font-mono bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded text-[10px]">{financialConfig?.supabaseTable || 'extra_incomes'}</code>
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mr-1">
            <span className="font-bold text-emerald-600">{syncedCount}</span> enviados &middot;{' '}
            <span className="font-bold text-amber-600">{unsyncedCount}</span> pendentes
          </div>
          ...
          <button
            ...
            onClick={() => handleSyncAll(true)}
            disabled={isSyncingAll || cashEntries.length === 0}
            className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 transition cursor-pointer shadow-2xs disabled:opacity-50"
          >
            {isSyncingAll ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5 fill-white" />}
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 font-semibold border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="py-3 px-4 text-right">Integração Financeira</th>
                </tr>
              </thead>
                      ...
                      <td className="py-3 px-4 text-right whitespace-nowrap">
                        {isZero ? (
                          ...
                          <div className="inline-flex items-center gap-1.5 justify-end">
                            ...
                            <button
                              type="button"
                              onClick={() => handleSyncSingle(entry.id)}
                              disabled={isSyncingId === entry.id}
                              className="text-[10px] text-slate-400 hover:text-emerald-600 underline font-medium cursor-pointer"
                              title="Reenviar este lançamento para o Controle Financeiro"
                            >
                              {isSyncingId === entry.id ? '...' : 'Reenviar'}
                            </button>
                          </div>
                        ) : (
                          <div className="inline-flex items-center gap-1.5 justify-end">
                            {entry.syncError ? (
                              <span
                                className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/60 px-2 py-0.5 rounded-md border border-rose-200 dark:border-rose-800/60 cursor-help"
                                title={`Erro no envio: ${entry.syncError}`}
                              >
                                <AlertCircle className="w-3 h-3 text-rose-600" />
                                Erro
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/60 px-2 py-0.5 rounded-md border border-amber-200 dark:border-amber-800/60">
                            ...
                            <button
                              ...
                              onClick={() => handleSyncSingle(entry.id)}
                              disabled={isSyncingId === entry.id}
                              className="px-2.5 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold flex items-center gap-1 transition cursor-pointer shadow-2xs disabled:opacity-50"
                              title="Enviar este valor para o Controle Financeiro agora"
                            >
                              {isSyncingId === entry.id ? (
      ...
      {/* Quick Supabase Connection & Sync Modal */}
      {isQuickSyncModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                    Conectar e Sincronizar com Controle Financeiro
                  </h3>
                  <p className="text-[11px] text-slate-500">
                    Envio direto para o banco de dados Supabase do seu sistema financeiro.
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setIsQuickSyncModalOpen(false)}
            ...
            <div className="bg-emerald-50 dark:bg-emerald-950/40 p-3 rounded-xl border border-emerald-200 dark:border-emerald-800 text-xs text-emerald-800 dark:text-emerald-300">
              <p className="font-bold flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                Lançamentos prontos para envio:
              </p>
              <p className="mt-1 text-[11px]">
                Será enviado o valor total do mês selecionado ({selectedMonth}):{' '}
                <strong className="text-emerald-700 dark:text-emerald-200 text-xs">
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalEffectiveCash)}
                </strong>{' '}
                como receita em <strong>Renda Extra (MASSOTERAPIA)</strong>.
              </p>
            </div>

            <form onSubmit={handleSaveQuickConfigAndSync} className="space-y-3.5 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                  <span>URL do Supabase</span>
                  <span className="text-[10px] text-slate-400 font-normal">Pré-configurada</span>
                </label>
                <input
                  type="url"
                  value={quickSbUrl}
                  onChange={e => setQuickSbUrl(e.target.value)}
                  className="w-full p-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 font-mono text-[11px]"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                  <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold">
                    ...
                    Chave de Acesso do Supabase (Anon Key ou Service Key) *
                  </span>
                  <span className="text-[10px] text-emerald-600 font-medium">Obrigatória</span>
                </label>
                <input
                  type="password"
                  value={quickSbKey}
                  onChange={e => setQuickSbKey(e.target.value)}
                  placeholder="Cole sua anon_key ou service_role_key do Supabase aqui..."
                  className="w-full p-2.5 rounded-lg border-2 border-emerald-500/80 dark:border-emerald-600 bg-white dark:bg-slate-800 font-mono text-[11px] focus:ring-2 focus:ring-emerald-500 outline-none"
                  required
                />
                <p className="text-[10px] text-slate-400">
                  Encontrada no seu painel Supabase em <strong>Project Settings &gt; API &gt; Project API keys</strong>.
                </p>
              </div>
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400 font-normal">Padrão do sistema</span>
                </label>
                <input
                  type="text"
                  value={quickSbTable}
                  onChange={e => setQuickSbTable(e.target.value)}
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold flex items-center gap-1.5 transition disabled:opacity-50 cursor-pointer shadow-xs"
                >
                  {isSavingQuickConfig ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ...
                    <>
                      <Zap className="w-4 h-4 text-amber-300 fill-amber-300" />
                      <span>Salvar Chave e Sincronizar Tudo</span>
                    </>
                  )}
                </button>
              </div>
