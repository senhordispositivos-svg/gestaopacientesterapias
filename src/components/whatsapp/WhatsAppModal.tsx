...
export const WhatsAppModal: React.FC<WhatsAppModalProps> = ({
  ...
  const copyUrlOnly = async () => {
    ...
    const ok = await copyToClipboard(validationUrl);
    if (ok) {
      setCopiedUrl(true);
          ...
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-700 space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={copyUrlOnly}
                className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[11px] font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1 hover:bg-slate-100 shrink-0 shadow-sm notranslate"
              >
                {copiedUrl ? (
                  <>
